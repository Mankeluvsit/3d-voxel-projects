...
interface StartScreenProps {
  onStart: (aiEnabled: boolean, gridSize: number) => void;
}
...
const StartScreen: React.FC<StartScreenProps> = ({ onStart }) => {
  ...
  const mapSizes = [
    { label: 'Small', size: 20, desc: '20x20' },
    { label: 'Medium', size: 40, desc: '40x40' },
    { label: 'Large', size: 60, desc: '60x60' },
  ];
    ...
    <div className="absolute inset-0 flex flex-col items-center justify-center z-50 text-white font-sans p-6 bg-black/30 backdrop-blur-sm transition-all duration-1000">
      <div className="max-w-md w-full bg-slate-900/90 p-8 rounded-2xl border border-slate-700 shadow-2xl backdrop-blur-xl relative overflow-hidden animate-fade-in">
        ...
        <div className="relative z-10">
            ...
            <div className="bg-slate-800/50 p-5 rounded-xl border border-slate-700/50 mb-6 hover:border-slate-600 transition-colors shadow-inner">
              <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-3 block">Select Map Size</label>
              <div className="grid grid-cols-3 gap-2">
                {mapSizes.map((m) => (
                  <button
                    key={m.label}
                    onClick={() => setGridSize(m.size)}
                  ...
                  >
                    ...
                    <span className="text-[10px] font-mono opacity-50">{m.desc}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            <button 
            onClick={() => onStart(aiEnabled, gridSize)}
